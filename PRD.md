...
- "Como podemos adaptar seus estudos?": o estudante marca uma ou mais **dificuldades reais** que encontra ao estudar — 🧠 me concentrar, 📖 ler e acompanhar textos, 🧩 entender conteúdos complexos, ✋ acompanhar muitas etapas. Não é diagnóstico, não classifica o usuário, não cita TDAH/dislexia/autismo nem qualquer condição médica. A adaptação muda só a **apresentação** — nunca a veracidade, as fórmulas, as respostas corretas, e **nunca elimina etapas ou conceitos** para "facilitar".
- Cada necessidade produz uma adaptação **distinta** no prompt (`lib/prompts.js` → `PREFERENCE_RULES`): concentração = uma ideia por vez; ler e acompanhar textos = frases curtas + disposição na tela (blocos, separação nítida), conteúdo e significado inteiros; conteúdos complexos = define o termo antes de usar, exemplo concreto, progressão; muitas etapas = uma etapa por linha, ação/porquê/resultado, todas as etapas preservadas.
- O usuário configura suas necessidades pelo ⚙️ do cluster central da câmera (não há popup automático ao abrir o app). Depois de cada captura, o passo "Sua captura" mostra a foto e as necessidades que serão usadas (pré-marcadas com o padrão); dá para ajustar só para aquela captura, ou marcar "Tornar meu padrão". Ambos usam o mesmo bottom sheet (`LearningPreferencesSheet`).
...
- "Ler e acompanhar textos" (`longText`) é a única que também muda a UI: `ContentBlocks` lê a preferência **atual** do usuário e aplica um perfil de leitura mais espaçado (entrelinha, respiro entre blocos, subtítulos destacados) a todo conteúdo, inclusive o antigo. Sem ela, a renderização é idêntica à anterior.
...
Fase 11: `concentration`/`longText`/`complexContent`/`manySteps`;
...
- Preferência **padrão** em `sv_db.learningPreferences`; preferência **de uma captura** é local da câmera. Acesso pelo ⚙️ e pelo passo "Sua captura" (que revisa as necessidades antes de cada análise) — sem popup automático ao abrir o app. Rastro opcional em `Content.learningPreferences`.
...
- As 4 preferências genéricas (Simplificar/Foco/Visual/Passo a passo) foram substituídas por **4 barreiras de aprendizagem declaradas**: 🧠 concentração, 📖 ler e acompanhar textos, 🧩 conteúdos complexos, ✋ muitas etapas. (Chegou a haver 5 — "ler textos longos" e "acompanhar textos" eram quase a mesma coisa e foram fundidas em uma.) A pergunta passou de "o que você prefere?" para "onde você encontra dificuldade?". Cada necessidade tem um foco distinto no prompt (`PREFERENCE_RULES`) — nenhuma vira "resumir".
...
- Carimbo `keysVersion` em `sv_db.learningPreferences` (`LEARNING_KEYS_VERSION` em `src/data/storage/db.js`, hoje `3`): bloco salvo com versão diferente da atual é invalidado no `readDb` — volta ao padrão de fábrica e o onboarding pergunta de novo, uma vez. Sem remapeamento automático das chaves antigas. `SCHEMA_VERSION` do db intacto. Conteúdo já salvo com o rastro antigo não é reescrito.
- `longText` ("ler e acompanhar textos") ativa um perfil de leitura mais espaçado em `ContentBlocks` (entrelinha, blocos, subtítulos), lido da preferência **atual** e válido para todo conteúdo. Sem ela, renderização byte a byte idêntica.
