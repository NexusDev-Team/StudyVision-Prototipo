...
export default function CapturedPageVisual({ content, height = 140 }) {
  const visual = getSubjectVisual(content.subjectName);
  const photo = content.images?.[0]?.dataUrl;
  return (
    <div style={{ position: "relative", height, borderRadius: 20, overflow: "hidden", background: `linear-gradient(135deg, ${visual.color} 0%, #1e1b4b 100%)`, boxShadow: `0 4px 20px ${visual.color}33` }}>
      ...
      {photo ? (
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ width: "70%", height: "82%", background: "#FAFAF7", borderRadius: 6, transform: "rotate(-4deg)", boxShadow: "0 14px 28px rgba(0,0,0,0.4)", padding: "10%", position: "relative" }}>
            <span style={{ position: "absolute", top: 8, right: 10, fontSize: 18, lineHeight: 1 }}>{visual.emoji}</span>
            {LINE_WIDTHS.map((w, i) => (
              <div key={i} style={{ height: 3.5, width: w, borderRadius: 2, marginBottom: 9, background: i === 0 ? visual.color : "#CBD5E1", opacity: i === 0 ? 0.85 : 0.55 }} />
            ))}
          </div>
        </div>
      )}
