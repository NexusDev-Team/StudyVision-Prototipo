            ...
            {subjects.map((s) => {
              ...
              return onSelect ? (
                <button
                  ...
                  style={{ display: "flex", width: "100%", textAlign: "left", background: "none", border: "none", borderBottom: "1px solid #F1F5F9", padding: "12px 0", cursor: "pointer", minHeight: 44 }}
                >
                  {row}
                </button>
              ) : (
                <div key={s.subjectId ?? s.name} style={{ padding: "12px 0", borderBottom: "1px solid #F1F5F9" }}>{row}</div>
              );
            })}
          </div>
        </Card>
