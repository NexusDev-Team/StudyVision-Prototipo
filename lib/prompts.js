...
// Formatação do "summary" quando Visual e/ou Passo a passo estão ativos. Os
// marcadores ("- " no início da linha, "1. " para etapas, linha SÓ com o
// subtítulo terminando em ":") são um CONTRATO com o renderizador de
...
const PREFERENCE_RULES = {
  ...
  visual: `- Visual: no "summary", organize com hierarquia visual — subtítulos, listas e
  separação explícita entre conceito, exemplo e aplicação (cada um como um
  subtítulo próprio). Cada subtítulo fica SOZINHO na sua linha, terminando em ":".
  Itens iniciados por "- ". Não use emoji decorativo.`,
  stepByStep: `- Passo a passo: no "summary", apresente o raciocínio como sequência numerada,
  UMA etapa por linha ("1. ", "2. ", ...). Para exercícios ou problemas, siga esta
  ordem: o que se pede, qual conceito usar, os passos, e só então o resultado —
  nunca revele o resultado antes dos passos. No máximo ~6 passos; não transforme
  tudo em um tutorial gigante.`,
};

// Chaves cujo resultado depende da formatação por linha do "summary".
const FORMATTING_KEYS = ["visual", "stepByStep"];
