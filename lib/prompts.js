...
export const PREFERENCE_KEYS = ["concentration", "longText", "complexContent", "manySteps"];

// Cabeçalho comum: as necessidades mudam a APRESENTAÇÃO, nunca o conteúdo.
// Exportado para ser reutilizado pelo Modo Foco (lib/focusPrompts.js) — a
// mesma regra de adaptação vale para qualquer prompt que respeite o Modo
// Inclusão, não só o de análise de imagem.
export const PREFERENCE_GUARDRAILS = `O estudante informou dificuldades que encontra ao estudar. Use-as para adaptar
...
// Formatação do "summary" quando longText / manySteps estão ativos. Os
// marcadores ("- " no início da linha, linha SÓ com o subtítulo terminando em
// ":") são um CONTRATO com o renderizador de
// src/components/study/ContentBlocks.jsx — o texto precisa vir com quebras de
// linha reais ("\n"), um bloco por linha.
//
// ATENÇÃO: os marcadores valem SÓ dentro do texto do campo "summary". O modelo
// lite tende a vazar "- " e frases sem aspas para o array "openQuestions", o que
// quebra o JSON (observado). Por isso o bloco reforça o escopo explicitamente.
//
// Etapas de processo NÃO usam esta lista genérica — ver STEP_FORMATTING
// abaixo, que é a única regra de formatação para "Etapa N:". Duplicar a
// proibição de lista numerada aqui (como havia antes) competia com a
// instrução positiva de "item de lista" logo acima dela, e o modelo lite
// seguia a positiva: por isso as etapas saíam empilhadas mesmo com manySteps
// ativo. Mantendo a regra de etapas num bloco único, sem instrução
// contraditória por perto, a formatação sai correta.
const SUMMARY_FORMATTING = `Formatação — vale APENAS para o texto do campo "summary", em nenhum outro campo:
  Dentro da string "summary", use quebras de linha reais ("\\n") entre os blocos:
  - Subtítulo: uma linha contendo APENAS o rótulo curto seguido de ":", nada de
    ...
    nas linhas seguintes. Ex.:
      ...
      - primeira ideia
      - segunda ideia
  - Item de lista: linha iniciada por "- ". Use isso para enumerar itens que
    NÃO são etapas de um processo (características, exemplos, causas, etc.).
  - Nunca junte rótulo e texto na mesma linha; nunca use "-" ou número no meio de
    uma frase corrida.

  Os demais campos NÃO usam esses marcadores. "openQuestions" continua sendo um
  array JSON de strings simples, cada uma entre aspas duplas, SEM "-", SEM
  numeração, SEM texto solto. "flashcards" e "quiz" mantêm sua estrutura JSON.
  O JSON de resposta deve permanecer estritamente válido.`;

// Formatação de ETAPAS dentro do "summary" — só entra quando manySteps está
// ativo. Regra isolada de propósito: é a ÚNICA instrução sobre como
// apresentar um processo com múltiplas etapas, sem nenhuma instrução positiva
// de lista por perto para competir com ela.
const STEP_FORMATTING = `Etapas de um processo, exercício ou demonstração dentro do "summary":
  NUNCA use lista numerada ("1. ", "2. ", ...) nem lista com marcador ("- ")
  para apresentar as etapas de um processo — isso empilha tudo, o exato
  problema que esta necessidade existe para evitar. Cada etapa é um SUBTÍTULO
  isolado em sua própria linha ("Etapa 1:", "Etapa 2:", ...), seguido do
  parágrafo daquela etapa nas linhas seguintes — nunca um item de lista. Ex.:
    Etapa 1:
    Isole o termo com x de um lado da igualdade.
    Etapa 2:
    Divida os dois lados pelo coeficiente de x.
  Preserve TODAS as etapas do processo original — nenhuma é pulada para
  encurtar.`;
...
//  longText       = tamanho das frases + disposição na tela (ler e acompanhar)
//  complexContent = profundidade / progressão da explicação
//  manySteps      = granularidade de processos e exercícios
// Exportado pelo mesmo motivo de PREFERENCE_GUARDRAILS acima — reuso no
// Modo Foco em vez de uma segunda cópia das regras por necessidade.
export const PREFERENCE_RULES = {
  ...
  longText: `- Dificuldade para ler e acompanhar textos: quebre paredes de texto em blocos
  curtos, cada um com um subtítulo próprio, parágrafos curtos e bem separados (uma
  ideia por bloco), com separação nítida entre título, conceito e explicação.
  Encurte as FRASES (frases longas viram duas ou três curtas). Nunca corte o
  conteúdo nem o significado — só a extensão da redação e a densidade visual
  diminuem, para a leitura fluir linha a linha.`,
  ...
  em etapas próprias e isoladas (a formatação exata de cada etapa está em regra
  à parte). Em cada etapa, separe claramente a ação, o porquê e o resultado
  parcial. Não entregue a resolução inteira de uma vez. Preserve TODAS as
  etapas — nenhuma é pulada para encurtar.`,
};

// Chaves cujo resultado depende da formatação por linha do "summary".
const FORMATTING_KEYS = ["longText", "manySteps"];
...
// Bloco comum "guardrails + regras ativas" — reutilizado por qualquer prompt
// que precise respeitar o Modo Inclusão (análise de imagem e Modo Foco).
// Devolve "" quando nenhuma necessidade está ativa. Não inclui
// SUMMARY_FORMATTING: aquele bloco é um contrato com o campo "summary" do
// Content, que só existe no fluxo de análise — cada prompt anexa sua própria
// regra de formatação por cima deste bloco comum.
export function buildPreferenceBlock(preferences) {
  const norm = sanitizePreferences(preferences);
  const active = PREFERENCE_KEYS.filter((key) => norm[key]);
  if (active.length === 0) return "";

  return `${PREFERENCE_GUARDRAILS}
...
export function buildAnalysisPrompt(preferences) {
  ...
  const needsStepFormatting = norm.manySteps === true;
